...
(function (window, angular, undefined) {
  ...
  angular.module('angularLocalStorage', ['ngCookies']).factory('Session', ['$parse', '$cookieStore', '$window', '$log', function ($parse, $cookieStore, $window, $log) {
    /**
     * Global Vars
     */
    ...
    var publicMethods = {
      ...
      StoreSession: function (credential) {
        if (!supported) {
          try {
            $cookieStore.put("token", credential);
            return credential;
          } catch (e) {
            $log.log('Local Storage not supported, make sure you have angular-cookies enabled.');
          }
        }
        var saver = angular.toJson(credential);
        storage.setItem("token", saver);
      ...
      bind: function ($scope, key, opts) {
        ...
        if (publicMethods.get(storeName) === null && typeof scopeVal === 'undefined') {
          publicMethods.save(storeName, opts.defaultValue);
        }
        ...
        watchers[watcherId] = $scope.$watch(key, function (val) {
          if (angular.isDefined(val)) {
            publicMethods.save(storeName, val);
          }
        }, true);
